...
Feature: Cadastro e Login de usuários
  ...
  Scenario Outline: Cadastro de usuário com sucesso
    Given que não existe um usuário cadastrado com email "<email>"
    When um novo usuário é criado com nome "<nome>", email "<email>", senha "<senha>" e data de nascimento "<nascimento>"
    And é criado um perfil de usuário com nome "<nome>", isKid "false" e avatar "<avatar>"
...
#    Given que existe um usuário cadastrado com email "<email>" e senha "<senha>"
#    When o usuário tenta fazer login com email "<email>" e senha "<senha>"
#    Then o login é bem-sucedido e o retorna o token de autenticação "<token>"
#    Examples:
#      | email            | senha     | token       |
#      | joao@example.com | Senha@123 | token123456 |
