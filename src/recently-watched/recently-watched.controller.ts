...
export class RecentlyWatchedController {
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Remover um vídeo da lista de assistidos recentemente',
  })
  @ApiParam({
    name: 'videoId',
    type: 'number',
    description: 'ID do Vídeo a ser removido',
  })
  @ApiQuery({
    name: 'profileId',
    type: 'number',
    required: true,
    description: 'ID do Perfil do qual o vídeo será removido',
  })
  @ApiResponse({
    status: 200,
    description: 'Vídeo removido com sucesso',
    schema: { example: { message: 'Vídeo removido com sucesso' } },
  })
